...
async function loadSettings() {
    try {
        ...
        if (settings.payment_qr) {
            ...
            // Check if it's a URL
            if (settings.payment_qr.startsWith('http')) {
                document.getElementById('qrUrlRadio').checked = true;
                document.getElementById('qrFileInput').style.display = 'none';
                document.getElementById('qrUrlInput').style.display = 'block';
                document.getElementById('paymentQrUrl').value = settings.payment_qr;
            } else {
                document.getElementById('qrFileRadio').checked = true;
                document.getElementById('qrFileInput').style.display = 'block';
                document.getElementById('qrUrlInput').style.display = 'none';
            }
        }
    } catch (error) {
...
// Toggle QR input methods
document.getElementById('qrFileRadio').addEventListener('change', function() {
    if (this.checked) {
        document.getElementById('qrFileInput').style.display = 'block';
        document.getElementById('qrUrlInput').style.display = 'none';
...
document.getElementById('qrUrlRadio').addEventListener('change', function() {
    if (this.checked) {
        document.getElementById('qrFileInput').style.display = 'none';
        document.getElementById('qrUrlInput').style.display = 'block';
...
document.getElementById('settingsForm').addEventListener('submit', async function(e) {
    ...
    // Check if using URL
    if (document.getElementById('qrUrlRadio').checked) {
        const url = document.getElementById('paymentQrUrl').value.trim();
        if (url) {
            formData.append('payment_qr_url', url);
        }
    } else {
        // File upload
        const fileInput = document.getElementById('paymentQr');
        if (fileInput.files.length > 0) {
            formData.append('payment_qr', fileInput.files[0]);
        }
    }
