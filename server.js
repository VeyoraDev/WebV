...
const dataDir = isRender ? '/opt/render/project/src/data' : '.';
...
app.use('/products', express.static(path.join(dataDir, 'products')));
...
db.serialize(() => {
    ...
    db.get(`SELECT * FROM products WHERE name = '100M Coin'`, (err, row) => {
        if (!row) {
            ...
            console.log('✅ Default product: 100M Coin');
        }
    });
    ...
    db.get(`SELECT * FROM products WHERE name = '200M Coin'`, (err, row) => {
        if (!row) {
            ...
            console.log('✅ Default product: 200M Coin');
        }
    });
});
...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        if (file.fieldname === 'receipt') {
            cb(null, path.join(dataDir, 'uploads'));
        } else if (file.fieldname === 'product_image') {
            cb(null, path.join(dataDir, 'products'));
