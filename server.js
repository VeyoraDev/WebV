...
// Create directories
['uploads', 'products', 'qr'].forEach(dir => {
...
db.serialize(() => {
    ...
    db.get(`SELECT * FROM settings WHERE admin_username = 'admin'`, async (err, row) => {
        if (!row) {
            ...
            db.run(`INSERT INTO settings (admin_username, admin_password, store_name) 
                    VALUES (?, ?, ?)`, ['admin', hashedPassword, 'VStore']);
        }
    });
...
app.post('/api/admin/settings', upload.single('payment_qr'), async (req, res) => {
    try {
        const { store_name, telegram_bot_token, telegram_owner_id, admin_username } = req.body;
        const payment_qr = req.file ? `/qr/${req.file.filename}` : null;
